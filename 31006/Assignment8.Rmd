...
```{r}
ts_cmeS_2013 <- filter(ts_cmeS_all, yearmon >= 2013)
ts_immS_2013 <- filter(ts_immS_all, yearmon >= 2013)
ts_iomS_2013 <- filter(ts_iomS_all, yearmon >= 2013)

ts_cmeS <- filter(ts_cmeS_all, yearmon < 2013)
ts_immS <- filter(ts_immS_all, yearmon < 2013)
ts_iomS <- filter(ts_iomS_all, yearmon < 2013)
```
...
#garch_cme <- garchFit(~garch(1,1), data=ts(ts_cmeS$spline), trace=F)
#summary(garch_cme)
#McLeod.Li.test(y=garch_cme$residuals)
```
...
#garch_cme <- ugarchfit(modelSpec, data=ts_cmeS$spline, solver='hybrid')
```
