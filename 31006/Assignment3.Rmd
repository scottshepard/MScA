...
quarters <- zoo::as.yearqtr(paste(df$Year, df$Quarter, sep="-"))

UN  <- xts::as.xts(df$UN,  order.by=quarters)
GDP <- xts::as.xts(df$GDP, order.by=quarters)
...
UN_arima <- forecast::auto.arima(UN['1955/1968'])
GDP_arima <- forecast::auto.arima(GDP['1955/1968'])
...
tseries::kpss.test(UN)
tseries::kpss.test(GDP)
```
