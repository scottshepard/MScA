...
df <- plyr::ldply(list.files(base_path), function(f) read_i57_xls(base_path, f))
head(df[,1:10])
```
...
time_index <- seq(from = as.POSIXct("2013-06-16 01:00"), 
                  to = as.POSIXct("2013-07-01 00:00"), by = "hour")

obs <- xts(df[df$date != '2013-07-01',5], order.by = time_index)
...
m0 <- auto.arima(obs)
m1 <- Arima(obs, c(4,0,6))
m2 <- Arima(obs, c(8,0,12))
...
```{r}
dow_arima <- auto.arima(obs, seasonal = )
forecast(m0, h=24)
```
