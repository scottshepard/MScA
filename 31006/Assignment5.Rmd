...
recursive_nonseasonal = forecast(m, h=12)
...
```{r}
m <- auto.arima(train)
recursive_seasonal = forecast(m, h=12)
plot(forecast(m, h=12))
```
...
m <- auto.arima(train_adj, seasonal = F) 
m_s <- auto.arima(train_adj) 
...
for( i in 1:12) {
  ...
  pf <- forecast(m, h=1)$mean
  pf_s <- forecast(m_s, h=1)$mean
  
  f_1990  <- c(f_1990,  pf)
  f_1990s <- c(f_1990s, pf_s)
  
  train_adj   <- ts(c(train_adj,   pf),   start=1975, frequency=12)
  train_adj_s <- ts(c(train_adj_s, pf_s), start=1975, frequency=12)
...
df <- data.frame(
  ...
  DirRec=f_1990, 
  DirRecS=f_1990s,
  Rec = recursive_nonseasonal$mean,
  RecS = recursive_seasonal$mean
  )
df
```
