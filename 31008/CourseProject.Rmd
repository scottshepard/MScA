...
lda_plot = accuracyPlot(holdout, "Genre.LDA") + 
  ggtitle("Accuracy of LDA in Holdout")
lda_plot
```
...
rf_plot = accuracyPlot(holdout, "Genre.RF") + 
  ggtitle("Accuracy of Random Forest in Holdout")
rf_plot
```
...
```{r}
gridExtra::grid.arrange(tree_plot, logit_plot, rf_plot, lda_plot)
```

The tree model yeilded the best, most consistent accuracy measure.
