...
accuracyTable <- function(df, pred.col) {
  group_by_(df, pred.col) %>%
    summarize_(pct=lazyeval::interp(~ mean(var == genre), var=as.name(pred.col)))
}
...
correctClassifyPlot(train, "Genre.Multilogit")
```

# LDA & QDA

```{r}
LDA.Model <- lda(genre ~ . , train[,c('genre','acousticness','danceability',
                                      'energy','instrumentalness','liveness',
                                      'speechiness','tempo','valence')])
train$Genre.LDA <- predict(LDA.Model)$class

QDA.Model <- qda(genre ~ . , train[,c('genre','acousticness','danceability',
                                      'energy','instrumentalness','liveness',
                                      'speechiness','tempo','valence')])
